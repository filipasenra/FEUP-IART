...
void draw_state(state st)
{
	...
	for (int y = HEI - 1; y >= 0; y--)
	{
		for (int x = 0; x < WID; x++)
		{
			if (st.board[y][x] == 0)
				cout << "|   ";
			else if (st.board[y][x] == 1)
				cout << "| X ";
			else if (st.board[y][x] == 2)
				cout << "| O ";
			if (x == WID - 1)
				cout << "|\n";
		}
...
movem get_pc_greedy_mov(state st)
{
	...
	for (mov.col = 0; mov.col < WID; mov.col++)
	{
		if (valid_movement(st, mov))
		{
			...
			if (evaluate(st2, st.player) > aval)
			{
				aval = evaluate(st2, st.player);
				movfin = mov;
			}
		}
	}
...
movem get_pc_minimax_mov(state st, int depth)
{
	movem movfin;	
	//...
	//Minimax with alpha beta cuts implementation - TODO
	//...
...
movem recursive_minimax(state st, int depth, bool isMax){
